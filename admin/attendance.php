...
if (isset($_GET['open_tt']) && isset($_GET['date'])) {
    ...
    $date = $_GET['date'];
    try {
        $db->prepare('INSERT IGNORE INTO att_sessions (timetable_id,session_date,opened_by) VALUES (?,?,?)')
           ->execute([$ttId, $date, $_SESSION['rep_id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'open_session') {
        ...
        try {
            $db->prepare('INSERT IGNORE INTO attendance (session_id,student_id,method) VALUES (?,?,\'manual\')')
        ...
        $sessionId = (int)($_POST['session_id'] ?? 0);
        $students  = $db->query('SELECT id FROM students WHERE is_active=1')->fetchAll();
        foreach ($students as $s) {
            $db->prepare('INSERT IGNORE INTO attendance (session_id,student_id,method) VALUES (?,?,\'manual\')')
               ->execute([$sessionId, $s['id']]);
        }
...
$dotDays = $db->query('SELECT DISTINCT day_of_week FROM timetable t JOIN courses c ON t.course_id=c.id WHERE c.is_active=1')->fetchAll(PDO::FETCH_COLUMN);

// Sessions that exist in this month (for calendar highlight)
$monthSessions = $db->prepare('
    SELECT s.session_date, COUNT(*) AS cnt
    FROM att_sessions s
    WHERE YEAR(s.session_date)=? AND MONTH(s.session_date)=?
...
$coursesForDate = $db->prepare('
    ...
    WHERE t.day_of_week=? AND c.is_active=1
    ...
    <?php else: ?>
      <div style="padding:6px 0">
        <?php foreach ($dateClasses as $cls):
          ...
          $ccolor = $colors[crc32($cls['code']) % count($colors)];
        ?>
