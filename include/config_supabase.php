<?php
// include/config.php  — Database configuration for both MySQL (local) and Postgres (Supabase)
...
function db(): PDO {
    ...
    // Check for Supabase/Postgres connection via DATABASE_URL env var
    ...
    if ($dbUrl) {
        ...
        try {
            ...
            $pdo->exec("SET timezone = 'Africa/Lagos'");
        } catch (PDOException $e) {
            http_response_code(500);
            die(json_encode(['error' => 'Postgres connection failed: ' . $e->getMessage()]));
        }
    } else {
        // Use local MySQL
        try {
