...
if ($studentId) {
    $s = db()->prepare('SELECT id, full_name, matric_no FROM students WHERE id=? AND is_active=TRUE');
    $s->execute([$studentId]);
    $student = $s->fetch();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['matric'])) {
        ...
        $s = db()->prepare('SELECT id, full_name, matric_no FROM students WHERE matric_no=? AND is_active=TRUE');
        $s->execute([$matric]);
        $student = $s->fetch();
