...
$stu = $db->prepare('SELECT id FROM students WHERE id=? AND is_active=1');
...
try {
    ...
    $db->prepare('INSERT IGNORE INTO attendance (session_id, student_id, method) VALUES (?, ?, ?)')
       ->execute([$sessionId, $studentId, 'qr']);
